"""EPO Open Patent Services (OPS) client — used for WIPO (WO-prefixed PCT
...
from a single IP can be considered excessive"). This is a legal
constraint, not just a technical one — Prompt.md section 7's instruction
to check "legal/access constraints" before building anything is answered
directly: scraping PATENTSCOPE would violate its terms outright.

EPO's OPS (https://developers.epo.org, free registration, OAuth2
client-credentials) is the legitimate machine-readable route instead. Its
INPADOC/DOCDB data covers WO-prefixed PCT publications' full bibliographic
data (applicants, inventors, priority/publication dates, IPC/CPC, family
id, title, abstract) — verified live on 2026-08-13.
...
CQL confirmed live: `pn=WO and (ti="..." or ab="...")`, `pd within
"YYYYMMDD,YYYYMMDD"` for a publication-date range (the comma-separated
form — a hyphenated range like "YYYYMMDD->YYYYMMDD" is rejected with
CLIENT.UnknownDateFormat). Range span is capped at 100 per request, and
total-result-count access is capped at 2000 across all pages of one query
(CLIENT.InvalidQuery beyond that) — confirmed live. Each of this job's
registered queries individually returns well under 2000 hits; a future
query addition must be checked against this cap before being registered.
...
retrieval=green:100"). Running this job's discovery step (5 queries,
...
class OPSClient:
    ...
    def fetch_biblio(self, docdb_id: str) -> bytes | None:
        """docdb_id like 'WO.2026163182.A1'. Returns None on OPS's genuine
        "no such publication" 404 (SERVER.EntityNotFound), not a fetch
        failure."""
