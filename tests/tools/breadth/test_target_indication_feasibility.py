...
def test_build_target_indication_rows_one_candidate_two_indications():
    ...
    for r in rows:
        ...
        assert r["supporting_asset_count"] == 1
        assert r["supporting_adc_candidates"] == "ADC_CANDIDATE_A"
...
def test_build_target_indication_rows_merges_evidence_for_shared_target_and_indication():
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["supporting_asset_count"] == 2
    assert row["supporting_adc_candidates"] == "ADC_CANDIDATE_A; ADC_CANDIDATE_B"
...
def test_build_target_indication_rows_never_asserts_validated_status():
    """Round-1 fix: an (ADC_TARGET, indication) pair proves an OBSERVED
    CLINICAL ASSOCIATION (a real record links this target's asset to this
    condition string) -- it does NOT prove target-in-indication
    therapeutic feasibility has been biologically validated. Must never
    be labeled status=VALIDATED."""
    candidate_rows = [_candidate("ADC_CANDIDATE_A", ["Breast Cancer"])]
    target_rows = [_target("ADC_TARGET_HER2", "HER2", ["ADC_CANDIDATE_A"])]

    rows = build_target_indication_rows(candidate_rows, target_rows)

    assert rows[0]["status"] == "OBSERVED_CLINICAL_ASSOCIATION"
...
def test_build_target_indication_rows_sorted_by_supporting_asset_count_then_target_then_indication():
    candidate_rows = [
        _candidate("ADC_CANDIDATE_A", ["Breast Cancer"]),
        _candidate("ADC_CANDIDATE_B", ["Breast Cancer"]),
    ...
    assert rows[0]["supporting_asset_count"] == 2
    assert rows[1]["supporting_asset_count"] == 1
